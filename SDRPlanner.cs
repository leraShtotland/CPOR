...
namespace CPOR
{
    ...
    class SDRPlanner
    {
        ...
        public enum Planners { FF, FFsa, FFha, MIPS, MetricFF, LPG, FD, CPT, FFX }
        ...
        public static Planners Planner = Planners.FFX;
        ...
        public static TimeSpan PlannerTimeout = new TimeSpan(0, 10, 0);
        ...
        public bool ActiveStrategyDeadEnds = false;
        ...
        public SDRPlanner(string sPath,string DeadEndFile,  Domain d, Problem p)
        {
            Data = new ExecutionData(sPath, DeadEndFile,  d, p);
        }
        List<string> Plan(string sPath,string deadEndFile,  PartiallySpecifiedState pssCurrent, int cPlans, out State sChosen,ref List<Action> cPlan)
        {
            ...
            for (int iPlan = 0; iPlan < cPlans; iPlan++)
            {
                ...
                List<string> lPlan = Plan(sPath, deadEndFile, pssCurrent, out sCurrentChosen,ref cPlan);
                lPlans.Add(lPlan);
        ...
        private List<string> PlanToObserveDeadEnd(string sPath, string deadEndFile, PartiallySpecifiedState pssCurrent, Formula maybeDeadEnd,bool ActiveStrategyDeadEnds,  out State sChosen, ref List<Action> cPlan)
        {
            DirectoryInfo di = new DirectoryInfo(sPath);
        ...
        int c = 0;
        private List<string> Plan(string sPath,string deadEndFile,  PartiallySpecifiedState pssCurrent, out State sChosen,ref List<Action> cPlan)
        {
            /*
            sChosen = null;
            ...
            */
            ...
            //lera
            // if we want to check out algoretem with dead ends and use WriteTaggedDomainAndProblem with the flag bOnlyIdentifyStates what is the List<State> lStates? what is cfGoal? the dead end? 



            //State state_dead= pssCurrent.WriteTaggedDomainAndProblem()
            ...
            if (false)
            {
                ...
                List<Action> lActions = BFSSolver.ManualSolve(pK, dK, true);
        ...
        public void OfflinePlanning(string sPath,string deadEndFile,  Domain domain, Problem problem)
        {
            ...
            while (!bDone)
            {
                ...
                while (lPlan == null || lPlan.Count == 0)
                {
                    ...
                    /***********************************************************************************
                   //find a deadend that may exist
                   //if a deadend surely exists, stop here and declare an "unsolvable" leaf
                   //if we find such a deadend - create a plan to learn whether the deadend exists
                   foreach (Formula daedend in problem.deadEndList) {
                       int isDeadEnd = pssCurrent.IsDeadEnd(daedend); 
                       if (isDeadEnd == 1)
                       {
                           Debug.WriteLine("dead end state is");
                           Console.WriteLine(daedend);
                       ...
                       // ||daedend.Size ==2 
                       {
                           lPlan = PlanToObserveDeadEnd(sPath, deadEndFile, pssCurrent, daedend, out sChosen, ref cPlan);
                       }
                   }
                   /***********************************************************************************/
                   ...
                   {
                        Console.WriteLine("close state enter");
                   ...
                   else
                   {
                       ...
                       if (lPlan == null)
                       {
                            ...
                            */
                            ...
                            // foreach (Formula daedend in problem.deadEndList)
                            //{
                            // resuceDeadEnd = daedend.Reduce(pssCurrent.m_lObserved);
                            //DeadEndExistence isDeadEnd = pssCurrent.IsDeadEndExistence(resuceDeadEnd);
                            Formula daedend = null;
                            Console.WriteLine(pssCurrent); 
                            DeadEndExistence isDeadEnd = pssCurrent.IsDeadEndExistenceAll(out daedend);
                                ...
                                // || daedend.Size == 2
                                {
                                    if (randomState == false)
                                    {
                                        ...
                                        lPlan = PlanToObserveDeadEnd(sPath, deadEndFile, pssCurrent, daedend, ActiveStrategyDeadEnds, out sChosen, ref cPlan);
                                        break;
                                    }
                                ...
                                if (isDeadEnd == DeadEndExistence.DeadEndTrue)
                                 {
                                     ...
                                     if (daedend is CompoundFormula)
                                     {
                                         ...
                                         daedend.GetAllPredicates(deadList);
                                     }
                                     else
                                     {

                                         PredicateFormula pf = (PredicateFormula)daedend;
                                    ...
                                    closeDead = true;
                                //break; 
                                  //  if (pssCurrent.deadEndList != null)
                               // {
                            ...
                            if (maybeDead==false && closeDead)
                           {
                            ...
                            /* option2 for deadend 
                            List<Formula> DeadEndList = pssCurrent.returnListOfDeadEnd();
                            if (DeadEndList.Count>0)
                            {

                                Debug.WriteLine("dead end state is");
                                Console.WriteLine(DeadEndList);
                                HashSet<Predicate> deadListPredicate = new HashSet<Predicate>();

                                foreach(Formula daedend in DeadEndList) { 
                                if (daedend is CompoundFormula)
                                {

                                    daedend.GetAllPredicates(deadListPredicate);
                                }
                                else
                                {

                                    PredicateFormula pf = (PredicateFormula)daedend;
                                    deadListPredicate.Add(pf.Predicate);
                                ...
                                pssCurrent.UpdateClosedStates(lClosedStates, alreadyVisitedStates, domain, deadListPredicate);
                                cFoundClosedStates++;
                                if (stateStack.Count == 0)
                                {
                                    bDone = true;
                            ...
                            List<Formula> MaybeDeadEndList = pssCurrent.returnListOfMaybeDeadEnd();
                            foreach (Formula daedend in MaybeDeadEndList)
                            {
                                lPlan = PlanToObserveDeadEnd(sPath, deadEndFile, pssCurrent, daedend, ActiveStrategyDeadEnds, out sChosen, ref cPlan);
                            ...
                            */
                            ...
                            if (lPlan == null && maybeDead==false && closeDead==false) {
                                 lPlan = Plan(sPath, deadEndFile,  pssCurrent, out sChosen, ref cPlan);
                 ...
                 if (lPlan != null)
                 {
                     ...
                     { 
                         ...
                         if (!IsReasoningAction(sAction.ToLower()))
                         {
                             ...
                             if (pssCurrent.IsClosedState(lClosedStates))
                             {
                                 Console.WriteLine("close state entry"+ pssCurrent);
        ...
        public void OnlineReplanning(string sPath,string deadEndFile,  Domain domain, Problem problem, out int cActions, out int cPlanning, out int cObservations, out TimeSpan tsTime)
        {
            ...
            */
                ...
                if (lPlan != null)
                {
                    ...
                    foreach (string sAction in lPlan)
                    {
                        ...
                        if (!IsReasoningAction(sAction.ToLower()))
                        {
                            ...
                            Formula daedend = null;
                            if (pssNext != null) { 
                            DeadEndExistence isDeadEnd = pssNext.IsDeadEndExistenceAll(out daedend);
                            if (isDeadEnd == DeadEndExistence.DeadEndTrue)
                            {
                                Debug.WriteLine("dead end state is");
                                Console.WriteLine(daedend);
            ...
            Console.WriteLine("cActions:"+ cActions);
            Console.WriteLine("number_of_actions:" + number_of_actions);
            Debug.WriteLine("Average time - " + tsTime.TotalSeconds * 1.0 / cActions);
            Debug.WriteLine("Total time - " + tsTime.TotalSeconds);
            Debug.WriteLine("*******************************************************************************");
        }
        ...
        //static int iSeed = 0;
        public void OnlineReplanningII(string sPath, string sDeadEndFile, Domain domain, Problem problem, out int cActions, out int cPlanning, out int cObservations, out TimeSpan tsTime)
        {
            DateTime dtStart = DateTime.Now;
            //RandomGenerator.Init(iSeed++);
            BeliefState bsInitial = problem.GetInitialBelief();//, bsCurrent = bsInitial, bsNext = null;
            ...
            while (!bGoalReached)
            {
                ...
                // || daedend.Size == 2
                {
                    lPlan = PlanToObserveDeadEnd(sPath, sDeadEndFile, pssCurrent, daedend, ActiveStrategyDeadEnds, out sChosen, ref cPlan);


                }
                if (lPlan==null)
                    lPlan = Plan(sPath,sDeadEndFile,  pssCurrent, out sChosen, ref cPlan);
                cPlanning++;
                sCurrent = sChosen;
                lChosen.Add(sChosen);
                bPlanEndedSuccessfully = true;
        ...
        public void Start()
        {
...
#endif
            {
                ...
                OfflinePlanning(Data.Path, Data.DeadEndFile, Data.Domain, Data.Problem);
                sw.Stop();
        ...
        public class ExecutionData
        {
            ...
            public ExecutionData(string sPath,string sDeadEndFile, Domain d, Problem p)
            {
                Domain = d;
                Problem = p;
                Path = sPath;
