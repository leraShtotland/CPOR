...
namespace CPOR
{
    public class BeliefState
    {
        ...
        */
        ...
        private List<Predicate> ChooseHiddenPredicatesII()
        {
            ...
            foreach (CompoundFormula cf in m_lHiddenFormulas)
            {
                ...
                {
                    bool bSuccess = SelectOneOf(lAssignment, cf.Operands);
                    if (!bSuccess)
                        return null;
                }
                ...
                {
                    throw new NotImplementedException("Need to implement adding all operands.");
                }
            }
            return lAssignment;
        }
        ...
        public State WriteTaggedDomainAndProblemDeadEnd(PartiallySpecifiedState pssCurrent, string sDomainFile, string sProblemFile, List<Action> lAppliedActions, Formula maybedeadEnd, bool Active, out int cTags, out MemoryStream msModels)
        {
            List<List<Predicate>> lChosen = ChooseDeadEndState(maybedeadEnd, Active);
            List<State> lStates = ApplyActions(lChosen, lAppliedActions);
            ...
            return WriteTaggedDomainAndProblem(pssCurrent, sDomainFile, sProblemFile, lStates, Active, out cTags, out msModels);
        }
        ...
        private List<List<Predicate>> ChooseDeadEndState(Formula maybeDeadEnd, bool Active)
        {

            List<List<Predicate>> predicates = new List<List<Predicate>>();
            List<Predicate> first = new List<Predicate>();

            List<Predicate> second = new List<Predicate>();
            if (Active)
            {
                if (maybeDeadEnd is CompoundFormula)
                {
                    
                    HashSet<Predicate> predictsList = new HashSet<Predicate>();
                    maybeDeadEnd.GetAllPredicates(predictsList);
                    List<Predicate> deadEndListFirst = new List<Predicate>();
                    HashSet<Predicate> pre = new HashSet<Predicate>();


                    foreach (Predicate p1 in predictsList)
                    {
                        if (Observed.Contains(p1)==false)
                            deadEndListFirst.Add(p1);
                    }
                    first = SimpleChooseHiddenPredicates(new List<Formula>(m_lHiddenFormulas), pre, deadEndListFirst, predicates,false);
                    List<Predicate> deadEndListSecond = new List<Predicate>();
                    foreach (Predicate p1 in first)
                    {   if (Observed.Contains(p1.Negate()) == false)
                            deadEndListSecond.Add(p1.Negate());
                    }
                    HashSet<Predicate> preSecond = new HashSet<Predicate>();
                    second = SimpleChooseHiddenPredicates(new List<Formula>(m_lHiddenFormulas), preSecond, deadEndListSecond, predicates,false);


                    //Formula orFormula = maybeDeadEnd.Negate();
                    //List<Formula> HiddenFormulasWithOrFormula = new List<Formula>(m_lHiddenFormulas);
                    //HiddenFormulasWithOrFormula.Add(orFormula);
                    //HashSet<Predicate> preSecond = new HashSet<Predicate>();

                    //second = SimpleChooseHiddenPredicates(HiddenFormulasWithOrFormula, preSecond, new List<Predicate>(), predicates);

                    // second.Add(predicts.First().Negate());
                ...
                {
                    List<Predicate> deadEndList = new List<Predicate>();
                    PredicateFormula pf = (PredicateFormula)maybeDeadEnd;
                    deadEndList.Add(pf.Predicate);
                    HashSet<Predicate> pre = new HashSet<Predicate>();
                    first = SimpleChooseHiddenPredicates(new List<Formula>(m_lHiddenFormulas), pre, deadEndList, predicates, false);
                    HashSet<Predicate> preSecond = new HashSet<Predicate>();
                    List<Predicate> deadEndListSecond = new List<Predicate>();
                    foreach (Predicate p1 in first)
                    {
                    ...
                    //  foreach (Predicate p1 in first)
                    // {
                    //if (Observed.Contains(pf.Predicate.Negate()) == false)
                    //  deadEndListSecond.Add(pf.Predicate.Negate());
                    //  }
                    ...
                    //second = SimpleChooseHiddenPredicates(new List<Formula>(m_lHiddenFormulas), preSecond, first, predicates);
                    ...
                    */
                    //predicates.Add(first);
                    //second= ChooseHiddenPredicates(new List<CompoundFormula>(m_lHiddenFormulas), predicates, true); 

                    //second.Add(pf.Predicate.Negate());

                    if (first.Contains(pf.Predicate))
                    {
                        predicates.Add(second);
                        predicates.Add(first);
                    }
            ...
            else
            {
                if (maybeDeadEnd is CompoundFormula)
                {
                    ...
                    HashSet<Predicate> predictsList = new HashSet<Predicate>();
                    maybeDeadEnd.GetAllPredicates(predictsList);
                    List<Predicate> NondeadEndListFirst = new List<Predicate>();
                    HashSet<Predicate> pre = new HashSet<Predicate>();

                    NondeadEndListFirst = new List<Predicate>();
                    foreach (Predicate p1 in predictsList)
                    {
                        if (Observed.Contains(p1.Negate()) == false) {
                            
                            NondeadEndListFirst.Add(p1.Negate());
                       
                        }
                    }
                    predicates.Add(NondeadEndListFirst);
                    //predicates.Add(NondeadEndListFirst); 
                    first = ChooseHiddenPredicates(new List<CompoundFormula>(m_lHiddenFormulas), predicates, true);
                    predicates = new List<List<Predicate>>();
                    predicates.Add(first);
                    second = ChooseHiddenPredicates(new List<CompoundFormula>(m_lHiddenFormulas), predicates, true);
                    predicates = new List<List<Predicate>>();
                    /*
                    if (first.Contains(predictsList))
                    {
                        predicates.Add(second);
                        predicates.Add(first);
                    }
                ...
                {
                    List<Predicate> NondeadEndList = new List<Predicate>();
                    PredicateFormula pf = (PredicateFormula)maybeDeadEnd;
                    NondeadEndList.Add(pf.Predicate.Negate());
                    predicates.Add(NondeadEndList);
