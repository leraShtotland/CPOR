...
namespace CPOR
{
    ...
    class SDRPlanner
    {
        ...
        public static Planners Planner = Planners.FD;
        ...
        public bool ActiveStrategyDeadEnds = true;
        ...
        public List<string> RunPlanner(string sPath, MemoryStream msModel, int iIndex)
        {
            if (SDRPlanner.UseFilesForPlanners)
            {
                ...
                if (Planner == Planners.FD)
                {
                    ...
                    return RunFDPlannerWithFiles(sPath, msModel, iIndex);
                }
        ...
        private List<string> RunFDPlannerWithFiles(string sPath, MemoryStream msModels, int iIndex)
        {
            ...
            pFD.StartInfo.FileName = @"C:\Python27\python.exe";
            ...
            pFD.StartInfo.Arguments += @"C:\Users\lera\Dropbox\SDR\DeadEnds\Planners\Fast-Downward-af6295c3dc9b\fast-downward.py";
            ...
            if (iIndex == -2)
                ...
                pFD.StartInfo.Arguments += " kd.pddl kp.pddl";
        ...
        public void OfflinePlanning(string sPath,string deadEndFile,  Domain domain, Problem problem)
        {
            ...
            while (!bDone)
            {
                ...
                while (lPlan == null || lPlan.Count == 0)
                {
                   ...
                   {
                        Console.WriteLine("cliseeStateeeee anterrrr!!!!!!!!!!!!!!!!!!!!!!!!!");
                       ...
                       if (lPlan == null)
                       {
                            ...
                            */
                            ...
                            closeDead = false; 
                             foreach (Formula daedend in problem.deadEndList)
                             {
                                 DeadEndExistence isDeadEnd = pssCurrent.IsDeadEndExistence(daedend);
                     ...
                     { 
                         ...
                         if (!IsReasoningAction(sAction.ToLower()))
                         {
                            ...
                            /*
                             if (pssCurrent.IsClosedState(lClosedStates))
                             {
                                 Console.WriteLine("closedstateee entry!!!!!!!!!!!!!!!!!!!!!!!!!");
        ...
        public void OnlineReplanningII(string sPath, string sDeadEndFile, Domain domain, Problem problem, out int cActions, out int cPlanning, out int cObservations, out TimeSpan tsTime)
        {
            ...
            while (!bGoalReached)
            {
                ...
                List<Action> cPlan = null;
                List<string> lPlan = Plan(sPath,sDeadEndFile,  pssCurrent, out sChosen, ref cPlan);
                cPlanning++;
                sCurrent = sChosen;
                lChosen.Add(sChosen);
                bPlanEndedSuccessfully = true;
