...
namespace PDDL
{
    class SDRPlanner
    {
        ...
        public enum Translations { SDR, MPSRTagPartitions, MPSRTags, BestCase, Conformant }

        private static Dictionary<Thread, Process> FFProcesses = new Dictionary<Thread, Process>();

        public static Planners Planner = Planners.CPT;
        ...
        public static TimeSpan PlannerTimeout = new TimeSpan(0, 0, 20);
        ...
        private bool RunPlannerWithFiles(string sPath, int iIndex)
        {
            ...
            {
                p.StartInfo.FileName = Program.BASE_PATH + @"\Planners\cpt.exe";
            ...
            while (!bDone)
            {
                ...
                while (lPlan == null || lPlan.Count == 0)
                {
                    ...
                    if (!pssCurrent.IsClosedState(dClosedStates))
                    {
                        ...
                        //Predicate pToObserve = pssCurrent.FindSimilarState(dClosedStates);
                        Predicate pToObserve = null;

                        cPlanning++;
                        lPlan = null; 
                        if (pToObserve != null)
                        {
                            /*List<Action> lActions = domain.GroundObservationAction(pToObserve, pssCurrent.Observed, true);
                            if (lActions.Count > 0)
                            {
                                Debug.WriteLine(" observing");
                                lPlan = new List<string>();
                                lPlan.Add(lActions[0].Name.Replace("_", " "));
                            }*/
                        ...
                        if (lPlan == null)
                        {
                            ...
                            if (true || pssInitial.MinMishapCount > 0)
                            {
                            ...
                            /*
                            if (lPlan == null && pssCurrent.MishapCount < pssCurrent.MinMishapCount)
                            {
                                pssCurrent.MinMishapCount = 0;
                                lPlan = Plan(sPath, pssCurrent, out sChosen);
                            }
                            */
                        }
                        ...
                        if (!IsReasoningAction(sAction.ToLower()))
                        {
                            ...
                            if (false)
                            {
                                ...
                                foreach (Action aObserve in lObservationActions)
                                {
                                    ...
                                    {
                                        pssCurrent.ApplyOffline(aObserve, out fObserved, out psTrueState, out psFalseState);
