...
namespace CPOR
{
    public class PartiallySpecifiedState
    {
        ...
        public PartiallySpecifiedState(PartiallySpecifiedState original)
        {
            ...
            if (original.deadEndList != null) { 

            foreach (Formula daedend in original.deadEndList)
            {
                ...
                if (daedend is PredicateFormula)
                {
                    ...
                    if (pre_deadEnd != Domain.FALSE_PREDICATE)
                    {
        ...
        public PartiallySpecifiedState(PartiallySpecifiedState sPredecessor, Action aGeneratingAction)
        {
         ...
         //   m_bsInitialBelief = new BeliefState(sPredecessor.m_bsInitialBelief);
            ...
            deadEndList = new List<Formula>();
            if (sPredecessor.deadEndList != null)
            {
                foreach (Formula daedend in sPredecessor.deadEndList)
                {
                    //deadEndList.Add(daedend);
        ...
        {
            foreach (Formula deadEnd in deadEndList)
            {
                if (deadEnd.IsTrue(m_lObserved))
        ...
        public DeadEndExistence IsDeadEndExistence(Formula deadEnd)
        {
            if (deadEnd.IsTrue(m_lObserved))
                return DeadEndExistence.DeadEndTrue;
            Formula deadEndNegate = deadEnd.Negate(); 
            if (deadEndNegate.IsTrue(m_lObserved))
                return DeadEndExistence.DeadEndFalse;
            Console.WriteLine("maybe dead end");
        ...
        public DeadEndExistence IsDeadEndExistenceAll(out Formula deadEnd)
        {
            deadEnd = null;
            ...
            foreach (Formula daedendTmp in DeadEndListCopy)
            {
                ...
                */
                ...
                Console.WriteLine("maybe dead end!!!");
                //Console.WriteLine(reduceDeadEnd);
                Console.WriteLine(daedendTmp);
                Console.WriteLine(this);
                deadendmaybe = true;
                if (deadEnd == null)
                {
                    deadEnd = reduceDeadEnd;
                    //deadEnd = daedendTmp;
                }
...
*/
        ...
        public State WriteTaggedDomainAndProblemDeadEnd(string sDomainFile, string sProblemFile, Formula maybeDeadEnd,  bool Active, out int cTags, out MemoryStream msModels)
        {
            return WriteTaggedDomainAndProblemDeadEnd(sDomainFile, sProblemFile, new List<Action>(), maybeDeadEnd, Active,   out cTags, out msModels);
        }
        ...
        private State WriteTaggedDomainAndProblemDeadEnd(string sDomainFile, string sProblemFile, List<Action> lActions,Formula maybeDeadEnd, bool Active,   out int cTags, out MemoryStream msModels)
        {
            PartiallySpecifiedState pssCurrent = this;
            while (pssCurrent.m_sPredecessor != null)
            {
            ...
            return m_bsInitialBelief.WriteTaggedDomainAndProblemDeadEnd(this, sDomainFile, sProblemFile, lActions, maybeDeadEnd, Active,  out cTags, out msModels);
        }
        ...
        public void UpdateClosedStates(List<PartiallySpecifiedState> lClosedStates, Dictionary<PartiallySpecifiedState,PartiallySpecifiedState> dVisited, Domain d, HashSet<Predicate> DeadEndList=null)
        {
            /***********************************************************************************
            ...
            if (DeadEndList != null)
            {
                m_lOfflinePredicatesKnown = DeadEndList;
                m_nPlan.isDeadEnd = true; 
