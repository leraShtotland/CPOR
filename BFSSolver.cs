...
namespace CPOR
{
    class BFSSolver
    {
        ...
        public static List<Action> ManualSolve(Problem p, Domain d, bool bApplyAllMerges)
        {
            State sStart = p.GetInitialBelief().ChooseState(true);
            State sCurrent = null, sNext = null;
            Dictionary<State, Action> dMapStateToGeneratingAction = new Dictionary<State, Action>();
        ...
        private static List<Action> GeneratePlan(State sBeforeLast, Action aLast, Dictionary<State, State> dParents, Dictionary<State, Action> dMapStateToGeneratingAction)
        {
            List<Action> lPlan = new List<Action>();
